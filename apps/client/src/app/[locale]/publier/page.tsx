...
const RecensezVotreAction = async (props: Props) => {
  ...
  const nbVues = (dispStatistics.nbVues || 0) + (dispStatistics.nbVuesMobile || 0);
  const nbFiches = (dispStatistics.nbDispositifs || 0) + (dispStatistics.nbDemarches || 0);
  const nbStructures = structStatistics.nbStructures;
  const isTablet = false;

  // write modal
  const showWriteModal = false;

  // active links
  const activeView = null;
  const inViewHero = false;
  const inViewWhy = false;
  const inViewRequired = false;
  const inViewSteps = false;
  const inViewFaq = false;
  const inViewRegister = false;
 ...
 */
  return (
    <div className={styles.main}>
      ...
      {/* HERO */}
      <div className={cls(styles.section, styles.bg_blue, "mb-4")}>
      ...
      <div className={styles.scrollspy_section}>
      ...
      <div className={cls(styles.section, styles.bg_grey, styles.scrollspy_section)}>
