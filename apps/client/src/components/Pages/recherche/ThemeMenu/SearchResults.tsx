import type { GetNeedResponse, GetThemeResponse } from "@refugies-info/api-types";
...
const SearchResults: React.FC = () => {
  ...
  const sectionData = useMemo(() => {
    const selectedNeeds = allNeeds.filter((need) =>
      need[locale].text.toLowerCase().includes(search.toLowerCase()),
    );
    const sortedThemes = themes.sort(sortThemes);
    return sortedThemes.reduce<SectionData[]>((acc, theme) => {
      return [
        ...acc,
        { theme, needs: selectedNeeds.filter((need) => need.theme._id === theme._id) },
      ];
    }, []);
  ...
  useEffect(() => {
    announce(t("Recherche.themeSearchResults", { count: needsCount, search }), {
      priority: "interrupt",
      delay: 1000,
    });
