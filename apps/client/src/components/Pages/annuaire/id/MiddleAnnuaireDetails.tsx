import type { GetStructureResponse, GetThemeResponse } from "@refugies-info/api-types";
...
const sortStructureActivities = (
  structure: GetStructureResponse | null,
  themes: GetThemeResponse[],
) => {
  const structureActivities: { title: string; themeName: string }[] = [];
  if (structure && structure.activities) {
    structure.activities.forEach((element) => {
      const detail = getActivityDetails(element, themes);
      const el = { title: element, themeName: detail.theme ? detail.theme.name.fr : "" };
      structureActivities.push(el);
    });
  }
  structureActivities.sort((a, b) => {
  ...
  const activitiesSortedByTheme: string[] = [];
          ...
          <div className={styles.title_container}>
            ...
            {structure.acronyme && (
              <h1 className={styles.title}>
                {structure.nom}{" "}
                <span style={{ color: "#828282" }}>{"- " + structure.acronyme}</span>{" "}
              </h1>
            )}
          </div>
          {hasUpdatePermission && (
            <div style={{ height: "5Opx" }}>
              <Link
                legacyBehavior
                href={getPath("/annuaire-creation", locale)}
                passHref
                prefetch={false}
              >
        ...
        <div className={styles.info_container}>
          <div className={styles.info_column_container}>
            ...
            <div className={styles.subtitle}>
              {t("Annuaire.phone_number", "Numéro de téléphone")}
            </div>
            <div className={styles.line_container}>
            ...
            <div className={styles.subtitle}>
              {t("Annuaire.departments", "Départements d'action")}
            </div>
            <div className={styles.line_container}>
              ...
              {(!structure.departments || structure.departments.length === 0) && (
                <Placeholder
                  ...
                  i18nKey="Annuaire.noDepartement"
                />
              )}
            </div>
          </div>
          <div className={styles.info_column_container}>
            <div className={styles.subtitle}>
              {t("Annuaire.reception_hours", "Horaires d'accueil")}
            </div>
            ...
            {structure.openingHours && structure.openingHours.noPublic && (
              <HoursPrecisions
                text={t("Annuaire.no_public", "Cette structure n'accueille pas de public.")}
              />
            )}
            {structure.onlyWithRdv && (
              <HoursPrecisions
                text={t("Annuaire.Uniquement sur rendez-vous", "Uniquement sur rendez-vous.")}
              />
            )}
            {structure.openingHours &&
              !structure.openingHours.noPublic &&
              weekDays.map((day) => (
                <DayHoursPrecisions day={day} openingHours={structure.openingHours} key={day} />
              ))}
          </div>
        </div>
