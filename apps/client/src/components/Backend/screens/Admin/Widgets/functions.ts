...
import type { UrlSearchQuery } from "~/pages/[locale]/recherche";
...
export const generateIframe = (widget: GetWidgetResponse) => {
  ...
  if (widget.typeContenu.length === 1) {
    // works only with 2 types of content
  ...
  const iframe = `<iframe width="930" height="600" src="${getBaseUrl()}fr/embed?${encodeURI(
    queryString,
  )}${analyticsParam}" title="${widget.name}" frameborder="0"></iframe>`;
