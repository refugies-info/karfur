...
export const getUsersToSendMail = (
  ...
  if (["En attente admin", "Accepté structure"].includes(status)) {
    ...
    const structureArray = structures.filter((structure) => structure._id === sponsorId);
    ...
    structure.membres.forEach((membre) => {
      ...
      if (!membreId) return;
      const membreWithDetailsArray = users.filter((user) => user._id === membreId);
...
export const getFormattedStatus = (dispoStatus: string | undefined) => {
  ...
  const corresStatus = correspondingStatus.filter((status) => status.storedStatus === dispoStatus);
