...
import {
  type fetchSelectedDispositifActionCreator,
...
export function* fetchSelectedDispositif(
  ...
  try {
    ...
    logger.info("[fetchSelectedDispositif] start fetching selected dispositif", {
      id: selectedDispositifId,
      locale,
    });
    if (selectedDispositifId) {
      const data: GetDispositifResponse = yield call(
        API.getDispositif,
