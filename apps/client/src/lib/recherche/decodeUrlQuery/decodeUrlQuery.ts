...
export const decodeQuery = (
  ...
  const {
    departments, needs, themes, age, frenchLevel, language, sort, type, search, status
  ...
  if (departments || needs || themes || age || frenchLevel || routerQuery.public || status || language || sort || type || search) {
    ...
    if (routerQuery.public) query.public = decodeURIComponent(routerQuery.public as string).split(",") as PublicOptions[];
