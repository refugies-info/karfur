...
export const hasMissingAccordions = (
...
export const getDefaultValue = (
  ...
  const defaultValues: UpdateDispositifRequest = {
    ...
    sponsors: (dispositif.sponsors as (Sponsor | string | ContentStructure)[])?.map((sponsor) => {
      ...
      if ((sponsor as ContentStructure)._id) return (sponsor as ContentStructure)._id.toString();
  ...
  if (defaultValues.how)
    defaultValues.how = addMissingAccordions(
      defaultValues.how,
      dispositif?.typeContenu || ContentType.DISPOSITIF,
      "how",
    );
  if (defaultValues.next)
    defaultValues.next = addMissingAccordions(
      defaultValues.next,
      dispositif?.typeContenu || ContentType.DISPOSITIF,
      "next",
    );
