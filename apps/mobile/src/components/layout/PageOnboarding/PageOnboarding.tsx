...
const PageContainer = styled.View<{ darkBackground: boolean }>`
  background-color: ${({ theme, darkBackground }) =>
    darkBackground ? theme.colors.dsfr_actionLowBlue : theme.colors.dsfr_backgroundBlue};
  flex: 1;
`;
...
const PageOnboarding = ({
  ...
  return (
    <PageContainer darkBackground={!!darkBackground}>
      {noScrollable ? (
        <View style={{ paddingBottom: insetBottom + TAB_BAR_HEIGHT }}>{children}</View>
      ) : (
        <ScrollableContent
      ...
      {!hideNavbar && (
        <BottomTabBar insetBottom={insetBottom}>
          {onPrevious && (
            <ButtonDSFR
              accessibilityLabel={t("onboarding_screens.back_button_accessibility")}
              onPress={onPrevious}
              priority="secondary"
              iconName="arrow-back"
