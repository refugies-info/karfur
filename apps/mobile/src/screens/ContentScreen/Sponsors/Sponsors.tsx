import type { ContentStructure, Sponsor } from "@refugies-info/api-types";
    ...
    <>
      ...
      <Rows>
        {sponsors.map((sponsor, index) => {
          const image = ((sponsor as Sponsor).logo || (sponsor as ContentStructure).picture)
            ?.secure_url;
          return (
            <Columns key={index} RTLBehaviour layout="auto 1" verticalAlign="center">
              ...
              <TextDSFR_MD>
                <ReadableText>
                  {(sponsor as Sponsor).name || (sponsor as ContentStructure).nom}
                </ReadableText>
              </TextDSFR_MD>
            </Columns>
          );
