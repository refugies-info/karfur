...
export const addTradToAirtable = async (
  ...
  return getAirtableTranslationTable("SUIVI TRAD").create([trad], { typecast: true }, (error: Error) => {
    if (error) {
...
export const addFeedbackToAirtable = async (
  ...
  return getAirtableTranslationTable("SUIVI bénévoles").create([feedback], { typecast: true }, (error: Error) => {
    if (error) {
