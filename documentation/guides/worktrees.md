...
`dev/` is a regular checkout of the `dev` branch. **Never commit work directly to it** — it's a stable reference point. Create a worktree for every piece of work.
...
Configure worktrunk to place the `dev` branch directly at `../dev` and all other worktrees in the hidden `.worktrees/` directory:

```toml
# ~/.config/worktrunk/config.toml
[projects."github.com/refugies-info/karfur"]
worktree-path = "{% if branch == 'dev' %}../dev{% else %}../.worktrees/{{ branch | sanitize }}{% endif %}"
```

The conditional ensures `dev` lands as a top-level directory (not buried inside `.worktrees/dev/`).
