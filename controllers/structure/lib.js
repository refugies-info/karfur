...
async function add_structure(req, res) {
  if (!req.body || (!req.body.nom && !req.body._id)) {
    ...
    if(structure._id){
      ...
      const isMembre = (((r.membres || []).find(x => x.userId === req.userId) || {}).roles || []).some(y => y==="contributeur");
      if(isAdmin){
        promise=Structure.findOneAndUpdate({_id: structure._id}, structure, { upsert: true , new: true});
