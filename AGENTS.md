...
1. **lint-staged** - Runs linters/formatters on staged files only:
   ...
   - `prettier --write` on `**/common.json` files (translation files)
2. **knip** - Detects unused files and exports in the codebase
...
⚠️ **Note**: Only bypass hooks if absolutely necessary. Hooks exist to maintain code quality.
