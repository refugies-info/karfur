...
export const insertInDispositif = async (
  ...
  locale: string,
  result: Dispositif
): Promise<{ insertedDispositif: Dispositif; traductorIdsList: any }> => {
  ...
  //         }
  //         c.contentTitle[locale] = traduction.translatedText.contenu[i].children[j].contentTitle;
  //       }
  //     });
  //   }
  // });

  // const translationsToAdd = traduction.traductions ? traduction.traductions.map((x: any) => x._id) : [];
  ...
  // const participantsToAdd = traduction.traductions ? traduction.traductions.map((x: any) => (x.userId || {})._id) : [];
