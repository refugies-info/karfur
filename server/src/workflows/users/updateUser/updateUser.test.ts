...
import { requestSMSlogin2FA, verifyCode } from "../../../modules/users/login2FA";
...
describe("updateUser", () => {
  ...
  it("should return 501 if user himself and send SMS for phone verification", async () => {
    requestSMSlogin2FA.mockImplementationOnce(() => { throw new Error("NO_CODE_SUPPLIED") });
    await updateUser(
      {
        fromSite: true,
        body: {
    ...
    expect(requestSMSlogin2FA).toHaveBeenCalledWith("0600000000");
