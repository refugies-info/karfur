...
const getFormattedAvancement = (avancement: Record<string, string> | string) => {
  // @ts-ignore
  if (!avancement.fr) return [];

  const langues = Object.keys(avancement);
  return langues.filter((langue) => langue !== "fr").map((langue) => getFormattedLocale(langue, "short"));
};
...
const addOrUpdateDispositifInContenusAirtable = async (
  ...
  try {
    ...
    if (id) {
      ...
      await base("CONTENUS").update([{ id, fields }], { typecast: true });
    } else {
...
const exportDispositifInAirtable = (
  ...
  const tagsList = tags ? tags.filter((tag) => !!tag).map((tag) => tag.short) : [];
  ...
  base("CONTENUS")
    ...
    .firstPage(async function (err: Error, records: any) {
      try {
        if (err) {
          logger.error("[exportDispositifInAirtable] error while getting dispositif in airtable", { error: err });
          return;
        }
...
export const updateAirtableContenus = async (req: RequestFromClient<{}>, res: Res) => {
  try {
