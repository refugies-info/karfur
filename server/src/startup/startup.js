  ...
  run = async (db) => {
    try {
      ...
      if (nb_documents < langues.length) {
        console.log(
          await db.collection("langues").insertMany(langues).insertedIds
        );
      }
      ...
      if (nb_documents < 10 && false) {
        let dispositifs = new Array(100).fill(dispositif).map((x, key) => {
          return {
            ...x,
        ...
        console.log(
          await db.collection("dispositifs").insertMany(dispositifs).insertedIds
        );
      }
...
const _insertI18nLocales = async () => {
  ...
  const currArticle = await Article.findOne({
    isStructure: true,
    title: "Structure du site",
    status: "Actif",
    canBeUpdated: true,
  });
  if (!currArticle || !currArticle.body) {
    console.log("aucun article retourné, fin.");
    return;
  }
  const localeFolder = __dirname + "/../client/src/locales";
  let frJson = JSON.parse(
    fs.readFileSync(localeFolder + "/fr/translation.json", "utf8")
  );
  let nbMots = 0;
  let avancement = { fr: 1 };
      ...
      try {
        fs.readdirSync(localeFolder + "/" + dir.name).forEach((file) => {
          if (file.includes(".json")) {
            ...
            var jsonLoc = JSON.parse(
              fs.readFileSync(
                localeFolder + "/" + dir.name + "/" + file,
                "utf8"
              )
            );
            avancement[dir.name] = 0;
            let tempObj = _insertNested(
              frJson,
              jsonLoc,
              dir.name,
              nbMots,
  ...
  if (nbMots > 0) {
    Object.keys(avancement).map((key) => {
      avancement[key] = avancement[key] / nbMots;
    });
  }
  let localeArticle = {
    title: "Structure du site",
...
_insertNested = (frJson, jsonLoc, locale, nbMots, avancement, currBody) => {
  Object.keys(frJson).forEach((key) => {
    if (
      typeof frJson[key] === "string" ||
      (frJson[key] && typeof frJson[key].fr === "string")
    ) {
      frJson[key] = frJson[key].fr
        ? frJson[key]
        : {
            fr: frJson[key],
            id: _.get(currBody, key + ".id") || uniqid("struct_"),
          };
      if (locale === "fr") {
      ...
      if (
        jsonLoc &&
        jsonLoc[key] &&
        (typeof jsonLoc[key] === "string" || jsonLoc[key] instanceof String)
      ) {
...
const _getI18nLocales = () => {
  Article.findOne(
    { isStructure: true, title: "Structure du site", status: "Actif" },
    (err, article) => {
      if (err || !article) {
      ...
      Object.keys(article.avancement).forEach((key) => {
        ...
        fs.writeFile(
          __dirname + "/../client/src/locales/" + key + "/translation.json",
...
const _mergeLocalesFiles = () => {
  ...
  if (nbMots > 0) {
    Object.keys(avancement).map((key) => {
      avancement[key] =
        key === "fr" ? avancement[key] : avancement[key] / nbMots;
    });
  }
  let localeArticle = {
    title: "Structure du site",
...
_mergeNested = (localJson, prodJson, nbMots, avancement) => {
  ...
  Object.keys(localJson).forEach((key) => {
    if (
      localJson[key] &&
      localJson[key].fr &&
      typeof localJson[key].fr === "string"
    ) {
...
const _recalculateAvancement = () => {
  Article.findOne(
    ...
    (err, article) => {
      if (err || !article) {
        return console.log(
          "Something went wrong when downloading data : " + err
        );
      }
      let nbMots = 0,
        avancement = { fr: 1 };
      ...
      if (nbMots > 0) {
        Object.keys(avancement).map((key) => {
          avancement[key] =
            key !== "fr" ? avancement[key] / nbMots : avancement[key];
    ...
    if (
      ...
      Object.keys(article[key]).forEach((locale) => {
        if (locale === "fr") {
          ...
          avancement[locale] =
            (avancement[locale] || 0) +
            article[key].fr.trim().split(/\s+/).length;
        }
      });
