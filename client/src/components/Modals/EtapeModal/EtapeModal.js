...
import { ModalBody, ModalFooter, ListGroup, ListGroupItem, Label, Input } from 'reactstrap';
...
class EtapeModal extends Component {
  ...
  handleCheck = (idx) => this.setState(pS => ({ checked: pS.checked.map((x,i) => i === idx ? !x : x) }));

  handleCtaClick = () => {
    const option = (this.props.subitem || {}).option || {};
    switch (option.texte) {
      case "En ligne":
        window.open((option.value1.indexOf("http") > -1 ? "" : "http://") + option.value1, '_blank');
        break;
      case "En physique":
        this.props.upcoming();
        break;
  ...
  render() {
    ...
    const displayedText = array1.includes(option.texte) ? option.value1 :
      option.texte === "Par courrier" ? [option.value1, option.value2, option.value3, option.value4].join(', ') :
      ...
      <Modal
        ...
        modalHeader = {<>
          <div className="title-wrapper">
            ...
            <span className="smaller-header">Étape #{subkey + 1} - {subitem.title}</span>
          </div>
          <ReactToPrint
            trigger={() => 
              <FButton type="light-action" name="printer-outline" fill={variables.noir} >
      ...
      >
        <ModalBody>
          ...
          <div className="info-wrapper" onClick={this.handleCtaClick}>
            <h5>
              {displayedText}
              <div className="copy-wrapper" onClick={e=>{e.stopPropagation();navigator.clipboard.writeText(displayedText);}}>
            ...
            <ListGroup>
              {subitem.papiers.map((papier, idx) => {
                if(checked[idx] !== undefined){
                  return (
                    <ListGroupItem key={idx} className={checked[idx] ? "checked" : "unchecked"}>
                      <Label check>
                        <Input type="checkbox" checked={checked[idx]} onChange={()=>this.handleCheck(idx)} />{' '}
