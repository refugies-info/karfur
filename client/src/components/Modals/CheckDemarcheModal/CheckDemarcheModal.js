...
class CheckDemarcheModal extends Component {
  ...
  componentDidMount(){
    if(this.state.step !== 0){this.setState({step: 0, checked: false})}
  }
  ...
  render() {
    ...
    return(
      <Modal 
        ...
        modalHeader={step === 0 ? t("CheckDemarche.Êtes-vous sûr", "Êtes-vous sûr ?") : t("CheckDemarche.Existe deja", "Est-ce qu’elle existe déjà ?")}
        {...this.props}
      >
        <ModalBody>
          {step === 0 ?
            <>
              <p>{t("CheckDemarche.Vous allez créer", "Vous allez créer une fiche démarche administrative")}. </p>

              <p>{t("CheckDemarche.Ce type de fiche", "Ce type de fiche concerne seulement les démarches proposées par l’État ou par une institution publique")}. </p>

              <p>
                {t("CheckDemarche.Exemple", "Exemple : la demande de logement social est une démarche administrative")}. <br/>
                {t("CheckDemarche.Le programme", "Le programme de service civique Volont'R est un dispositif")}.
              </p>

              <div className="blocs-choix">
                <NavLink to="/dispositif" className="bloc-choix">
                  <h5>{t("Non", "Non")}</h5>
                  <span dangerouslySetInnerHTML={{ __html: t("CheckDemarche.Finalement", "Finalement je préfère plutôt créer une fiche <b>dispositif</b>") }} />
                </NavLink>
                <div className="bloc-choix bloc-droit" onClick={()=>this.setStep(1)}>
                  <h5>{t("Oui", "Oui")}</h5>
                  <span dangerouslySetInnerHTML={{ __html: t("CheckDemarche.je suis sur", "Je suis sûr de vouloir créer une fiche <b>démarche administrative</b>") }} />
                </div>
              </div>
            </> : 
            <>
              ...
              <FormGroup check className={"no-match cursor-pointer" + (checked ? " checked" : "")}>
                <Label check className="cursor-pointer">
                  ...
                  <span>Aucune fiche ne correspond à ma démarche administrative</span>
                </Label>
              </FormGroup>
            </> }
        </ModalBody>
...
export default connect(mapStateToProps)(
  withTranslation()(CheckDemarcheModal)
);