...
const mapModal = (props) => {
  return(
    <Modal isOpen={props.showModal} toggle={props.toggleModal} className="map-modal">
      <ModalHeader toggle={props.toggleModal}>
        Ajouter des coordonnées
      </ModalHeader>
      <ModalBody>
        <h6>Etape 1 : télécharger le modèle de données</h6> 
        <p><i>Ce fichier .csv s’ouvre avec Excel, vous n’aurez plus qu’à copier-coller les données correspondantes depuis votre  fichier de coordonées.</i></p>
        ...
        <p><i>Une fois votre fichier modèle complété, téléversez-le ci-dessous</i></p>
        <CSVReader
      ...
      <ModalFooter>
        <Button color="success" onClick={props.toggleModal}>Valider</Button>
      </ModalFooter>
    </Modal>
  )
}
