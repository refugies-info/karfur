...
const parkourNow = (props) => {
  return (
    <div className="parkour-now slide-in-bottom">
      ...
      <Row>
        {props.demarches
          .slice(props.etape, props.etape + 3)
          .map((demarche, key) => {
            return (
              <Col xs="12" sm="4" md="3" className="card-col" key={key}>
                <CustomCard>
                  <CardBody>
                    <div
                      className={
                        "ribbon" + (demarche.bookmarked ? " active" : "")
                      }
                      onClick={() =>
                        props.setBookmark(key, "demarche", props.etape)
                      }
                    />
                  ...
                  <CardFooter className={"align-right bg-" + randomColor()}>
                    {demarche.title.substr(0, 35) +
                      (demarche.title.length > 35 ? "..." : "")}
                    <FormGroup check className="container">
                      <Input
                        ...
                        name={"checkbox" + key}
                        value={"checkbox" + key}
                        checked={demarche.checked}
                        onChange={() => props.checkItem(key, "demarche")}
                      />
                      <span
                        className="checkmark"
                        onClick={() =>
                          props.checkItem(key, "demarche", props.etape)
                        }
                      ></span>
                    </FormGroup>
                  </CardFooter>
        ...
        <Col xs="12" sm="2" md="1" className="card-col">
          <NavLink to="/">
            <i className="fa fa-chevron-right more-icon"></i>
          </NavLink>
        </Col>
      </Row>
      <h2>Dispositifs :</h2>
          ...
          return (
            ...
            >
              <CustomCard>
                ...
                <CardFooter className={"align-right bg-" + randomColor()}>
                  {dispositif.titreMarque.substr(0, 35) +
                    (dispositif.titreMarque.length > 35 ? "..." : "")}
                  <FormGroup check className="container">
                    <Input
                      className="form-check-input"
                      type="checkbox"
                      ...
                      name={"checkbox" + key}
                      value={"checkbox" + key}
                      checked={dispositif.checked}
                      onChange={() => props.checkItem(key, "dispositif")}
                    />
                    <span
                      className="checkmark"
                      onClick={() => props.checkItem(key, "dispositif")}
                    ></span>
                  </FormGroup>
                </CardFooter>
              </CustomCard>
                ...
                <CardFooter className={"align-right bg-" + randomColor()}>
                  ...
                  <FormGroup check className="container">
                    <Input
                      ...
                      onChange={() => props.checkItem(key, "article")}
                    />
                    <span
                      className="checkmark"
                      onClick={() => props.checkItem(key, "article")}
                    ></span>
                  </FormGroup>
                </CardFooter>
              </CustomCard>
