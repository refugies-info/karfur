...
const contenuDispositif = (props) => {
  ...
  return(
    props.menu.map((item, key) => {
      const newDisableEdit = disableEdit || (props.typeContenu==="demarche" && inVariante && !props.uiArray[key].varianteSelected);
      if((newDisableEdit && !inVariante) || props.typeContenu !== "demarche" || item.title !== "C'est pour qui ?"){
        return ( 
          <div 
            key={key} 
            className={'contenu-wrapper' + 
              (inVariante && disableEdit && item.content!==null && item.content!=="null" ? 
                (" in-variante" + (props.uiArray[key].varianteSelected ? " variante-selected" : "")) : "")} 
            id={"contenu-" + key}
          >
            {inVariante && disableEdit && item.content!==null && item.content!=="null" && 
              <Col className={"variante-radio" + (key !== 0 ? " mt-20": "")}>
                <div className="radio-btn" onClick={()=>props.updateUIArray(key, null, "varianteSelected", !props.uiArray[key].varianteSelected)}>
                  {props.uiArray[key].varianteSelected && <div className="active-inset" />}
                </div>
              </Col>}
