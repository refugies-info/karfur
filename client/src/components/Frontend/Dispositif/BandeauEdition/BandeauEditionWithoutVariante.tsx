...
const getInfoText = (step: number, displayTuto: boolean, typeContenu: "dispositif" | "demarche") => {
  const initialText = {
  ...
  if (step === -7)
    return {
      title: "Partenaires",
      subtitle: "Indiquez la structure responsable de la fiche ainsi que d'éventuelles structures associées."
    };
  if (typeContenu === "dispositif") {
    if (step === -4)
      return {
        title: "Titre informatif",
        subtitle: "Rédigez une courte phrase qui décrit l'action principale de votre dispositif."
      };
    if (step === 1)
      return {
        title: "Critères d'accès",
        subtitle: "Configurez les critères et conditions d'accès à votre dispositif."
      };
    if (step === -3)
      return {
    ...
    if (step === 2)
      return {
        ...
        subtitle: "Rédigez plusieurs arguments pour valoriser l'intérêt de votre dispositif."
      };
    if (step === -5)
      return {
  ...
  if (typeContenu === "demarche") {
    ...
    if (step === 0)
      return {
        title: "C'est quoi ?",
        subtitle: "Résumez la démarche en deux ou trois paragraphes synthétiques."
      };

    if (step === 1)
      return {
        title: "C'est pour qui ?",
        subtitle: "Configurez les critères et conditions d'accès à la démarche."
      };
    ...
    if (step === 3)
      return {
        title: "Et après ?",
        subtitle: "Abordez la fin ou le renouvellement du droit ou de la prestation obtenus."
      };
  }
...
export const BandeauEditionWithoutVariante = (props: Props) => {
  const { title, subtitle } = getInfoText(props.tKeyValue, props.displayTuto, props.typeContenu);
  const isDispositif = props.typeContenu === "dispositif";
  const correctContenu = props.typeContenu === "dispositif" ? "Dispositif" : "Démarche";
  return (
    <div className={`${styles.container} ${props.visible ? "" : styles.top}`}>
      ...
      >
        <div className={styles.group_container}>
          ...
          {props.tKeyValue !== -1 && props.displayTuto && <p className={styles.description}>{subtitle}</p>}
          {(props.tKeyValue === -1 || !props.displayTuto) && (
            <div className={styles.group_container}>
              <span className={styles.yellow_text}>jaune.</span>
              {isDispositif && (
                <div className={styles.more} onClick={props.toggleDispositifCreateModal}>
                  <u>En savoir plus</u>
                </div>
              )}
            </div>
        ...
        <div className={styles.group_container}>
          ...
          <FButton
            type={props.isModified ? "modified" : props.isSaved ? "saved" : "light-action"}
            name="save-outline"
            className="me-2"
            onClick={props.toggleDraftModal}
          >
            {props.isSaved && !props.isModified ? "Sauvegardé !" : "Sauvegarder"}
          </FButton>
          <FButton
            className="me-4"
            type="validate"
