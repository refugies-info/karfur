...
const membersTable = (props) => {
  ...
  data = data.map((x) => ({
    ...
    structRole: (x.roles || []).includes("administrateur")
      ? "Responsable"
      : (x.roles || []).includes("contributeur")
      ? "Rédacteur"
      : "Membre simple",
  }));
    ...
    <Table responsive className="avancement-user-table">
      ...
      <tbody>
        {data.map((element, key) => {
          const joursLastC =
            (new Date().getTime() -
              new Date(element.last_connected).getTime()) /
            (1000 * 3600 * 24);
          return (
            <tr key={key}>
              <td className="align-middle">
                <img
                  src={
                    element.picture ? element.picture.secure_url : marioProfile
                  }
              ...
              <td className="align-middle">
                <FButton
                  type="light-action role-btn"
                  onClick={() => props.editMember(element)}
                >
                  {element.structRole}
              ...
              <td className="align-middle hideOnPhone">
                {element.connected ? (
                  <span className="depuis success">Actuellement en ligne</span>
                ) : element.last_connected ? (
                  <span>
                    {moment(element.last_connected).calendar()}{" "}
                    <span
                      className={
                        "depuis " + (joursLastC > 3 ? "alert" : "success")
                      }
                    >
        ...
        {props.limit && dataArray.length > 5 && (
          <tr>
            <td
              ...
              onClick={() => props.toggleModal("members")}
            >
        ...
        <Row>
          ...
          {(
            ...
            <Col
              ...
              sm="12"
              xs="12"
              className="tableau-header no-margin"
            >
              <FButton
                type="dark"
                name="options-2-outline"
                onClick={() => props.toggleModal("addMember")}
              >
                Ajouter un membre
              </FButton>
            </Col>
