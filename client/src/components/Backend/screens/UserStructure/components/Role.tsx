...
const MainContainer = styled.div`
  ...
  border-width: 1px;
  border-style: solid;
  border-color: ${(props) => (props.state === "selected" ? colors.focus : colors.gray90)};
  box-sizing: border-box;
  border-radius: 12px;
  padding: 16px;
  margin: 4px 0px 4px 0px;
  cursor: pointer;
  color: ${(props) => (props.state === "selected" ? colors.white : colors.gray90)};
`;
...
export const Role = (props: Props) => (
  ...
  >
    <Row>
      ...
      <EVAIcon
        name={"radio-button-" + (["selected", "current"].includes(props.state) ? "on" : "off")}
