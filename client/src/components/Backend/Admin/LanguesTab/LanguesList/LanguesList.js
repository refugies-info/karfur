...
const languesList = (props) => {
  const langue_backup = (langue) => {
    if(langue.langueBackupId && langue.langueBackupId !== "undefined"){
      return props.langues.find(item => item._id === langue.langueBackupId).langueFr
        ...
        <Table responsive hover>
          ...
          <tbody>
            {props.langues.map((langue, index) =>
              <tr key={langue._id.toString()} onClick={() => props.onSelect({langue : langue})}>
                <th scope="row">{langue.langueFr}</th>
                <td><i className={"flag-icon flag-icon-" + langue.langueCode} title={langue.langueCode}></i></td>
                <td>{langue.i18nCode}</td>
                <td>{langue_backup(langue)}</td>
                <td><Badge color={colorStatut(langue.status)}>{langue.status}</Badge></td>
              </tr>
            )}
          </tbody>
        </Table>
