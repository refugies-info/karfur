...
const themesList = (props) => {
  const theme_under = (theme) => {
    if(theme.themeUnderId && theme.themeUnderId !== "undefined"){
      return props.themes.find(item => item._id === theme.themeUnderId).themeNom
        ...
        <Table responsive hover>
          ...
          <tbody>
            {props.themes.map((theme) =>
              <tr key={theme._id.toString()} onClick={() => props.onSelect({theme : theme})}>
                <th scope="row">{theme.themeNom}</th>
                <td>{theme.themeDescription.substring(0, Math.min(theme.themeDescription.length, maxDescriptionLength)) + (theme.themeDescription.length>maxDescriptionLength ? '...' : '')}</td>
                <td>{theme_under(theme)}</td>
                <td><Badge color={colorStatut(theme.status)}>{theme.status}</Badge></td>
              </tr>
            )}
          </tbody>
        </Table>
