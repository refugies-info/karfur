...
describe("async actions", () => {
  it("should fetch languages data", () => {
    ...
    return store.dispatch(actions.fetch_langues()).then(() => {
  ...
  it("should fetch user data", () => {
    //Pas testé le cas où l'utilisateur n'est pas connecté, à voir
    const store = mockStore({});
    return store.dispatch(actions.fetch_user()).then(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property("type");
      ...
      assert.include(
        actions[0].value,
        { status: "Actif" },
        "User is not connected. try logging in your browser and relaunch the test"
      );
  ...
  it("should fetch dispositifs", () => {
    const store = mockStore({});
    return store.dispatch(actions.fetch_dispositifs()).then(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property("type");
  ...
  it("should fetch structures", () => {
    const store = mockStore({});
    return store.dispatch(actions.fetch_structures()).then(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property("type");
...
describe("mock async actions", () => {
  ...
  it("should mock languages data", () => {
    ...
    return store
      .dispatch(actions.fetch_langues())
  ...
  it("should mock dispositifs", () => {
    ...
    moxios.wait(function () {
      let request = moxios.requests.mostRecent();
      expect(moxios.requests.mostRecent().config.url).to.contain(
        "/dispositifs/get_dispositif"
      );
      request.respondWith({
        status: 200,
        response: { data: fakeData },
  ...
  it("should mock structures", () => {
    ...
    return store
      .dispatch(actions.fetch_structures())
      .then(() => expect(store.getActions()[0]).to.deep.equal(expectedAction));
  });
});
