  ...
  getCookies = () => {
    ...
    if(nbLoaded===2){
      API.get_user_info().then(data_res => {
        ...
        this.setState({
          etapes:data.map((x,id)=> {return {...x, active:park.etapes[id].active}}),
          dispositifs:[...this.state.dispositifs].map((x,id)=> {return {...x, checked:park.dispositifs[id].checked, bookmarked:park.dispositifs[id].bookmarked}}),
          articles:[...this.state.articles].map((x,id)=> {return {...x, checked:park.articles[id].checked, bookmarked:park.articles[id].bookmarked}}),
          demarches:[...this.state.demarches].map((x,id)=> {return {...x, checked:park.demarches[id].checked, bookmarked:park.demarches[id].bookmarked}}),
        },()=>{nbLoaded++;})
      },function(error){console.log(error);return;})
    }
  }
        ...
        {this.state.etapes.map((etape, key) => {
          return (
            <div className={"timeline-item" + (etape.active ? " active":"")} key={key}>
              ...
              {etape.active && <ParkourNow checkItem={this.checkItem} setBookmark={this.setBookmark} etape={key} {...this.state}/>}
            </div>
          )}
        )}
      </div>
