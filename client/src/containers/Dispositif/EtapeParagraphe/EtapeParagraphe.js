...
class EtapeParagraphe extends Component {
  state= {
    options: this.props.demarcheSteps.options.map(x => ({...x, selected: false})),
  ...
  addDoc = (papier, idx=null, add=true) => {
    const newPapiers = idx===null ? [...(this.props.subitem.papiers || []), papier] : 
      add ? this.props.subitem.papiers.map((x,i) => i === idx ? papier : x) :
        this.props.subitem.papiers.filter((_,i) => i!==idx);
    
    this.setPropsValue(newPapiers, "papiers")
    this.setState(pS => ({ isPapiersDropdownOpen: [...pS.isPapiersDropdownOpen, false] }))
  }
        ...
        <Row className="relative-position">
          <Col lg="12" md="12" sm="12" xs="12" className="accordeon-col">
            <div className="title-bloc">
              <div 
                id="accordion-header"  
                className={"position-relative text-left " + (safeUiArray(keyValue, subkey, 'accordion') ? "active": "inactive")} 
                onMouseUp={() => disableEdit && updateUIArray(keyValue, subkey, 'accordion', !safeUiArray(keyValue, subkey, 'accordion'))} 
                ...
                <h5>
                  ...
                  {disableEdit && 
                    <EVAIcon name={"chevron-" + (safeUiArray(keyValue, subkey, 'accordion') ? "up" : "down") + "-outline"} size="large" fill={variables.darkColor} />}
                </h5>
            ...
            <Collapse className="bloc-configuration" isOpen={!disableEdit && configurationOpen} data-parent=".etapes-data">
              ...
              <div className={"row-config direction-colonne mb-10" + (validatedRow[0] ? " validated" : "")}>
                <div className="options-wrapper">
                  {options.map((option, key) => (
                    <div className={"col-config mr-10" + (option.selected ? " active" : "")} key={key} onClick={()=>this.selectOption(key, option)}>
                      <EVAIcon name={option.logo + "-outline"} fill={variables.noir} className="mr-10" />
                      <span>{option.texte}</span>
                    </div>
                  ))}
                </div>
                {isOptionSelected &&
                  <Form onSubmit={this.validateOption} className="mt-10 full-width">
                    <FormGroup>
                      <Label for="link" lg={selectedOption.label1 ? (selectedOption.texte === "Autre" ? "9" : "3") : "0"}>{selectedOption.label1}</Label>
                    ...
                    {selectedOption.placeholder2 && 
                      <FormGroup>
                        <Label for="displayText" lg={selectedOption.label2 ? "3" : "0"}>{selectedOption.label2}</Label>
                        <Col lg={selectedOption.label2 ? "9" : (selectedOption.placeholder3 ? "3" : "12")}>
                          <FInput
                            ...
                            id="displayText" type={selectedOption.icon2 ? "text" : "textarea"} placeholder={selectedOption.placeholder2} />
                        </Col>
                        {selectedOption.placeholder3 && <>
                          <Col lg="6">
                            <FInput
                              ...
                              id="displayText" type="text" placeholder={selectedOption.placeholder3} />
                          </Col>
                          <Col lg="3">
                            <FInput
                              prepend
                              ...
                              id="displayText" type="text" placeholder={selectedOption.placeholder4} />
                          </Col></>}
                      </FormGroup>}
                    <div className="footer-form">
                      {selectedOption.checkbox ? 
                        <FormGroup check className={"no-info" + (checked ? " checked" : "")}>
                          <Label check>
                      ...
                      <div className="footer-btns">
                        <FButton type="light-action" name="close-outline" className="mr-10" onClick={()=>this.selectOption()}>
                          Annuler
                        </FButton>
                        <FButton 
                          type="validate" 
              ...
              <div className={"row-config mb-10" + (validatedRow[1] ? " validated" : "")}>
                <EVAIcon name="clock-outline" fill={variables.noir} className="mr-10" />
                ...
                <ButtonDropdown isOpen={isDropdownOpen[0]} toggle={()=>this.toggleDropdown(0)} className="etape-dropdown mr-10">
                  <DropdownToggle caret>
                    {subitem.timeStepDuree}
                  </DropdownToggle>
                  <DropdownMenu>
                    {demarcheSteps.timeSteps.map((timeStep, key) => (
                      <DropdownItem key={key} id={key} onClick={()=>this.setPropsValue(timeStep.texte, 'timeStepDuree')}>
                        {timeStep.texte}
                      </DropdownItem>
                    ))}
                  </DropdownMenu>
                </ButtonDropdown>
                <span className="color-grisFonce">Précisez le temps nécessaire pour faire cette étape.</span>
              </div>

              <div className={"row-config mb-10" + (validatedRow[2] ? " validated" : "")}>
                ...
                <ButtonDropdown isOpen={isDropdownOpen[1]} toggle={()=>this.toggleDropdown(1)} className="etape-dropdown mr-10">
                  <DropdownToggle caret>
                    {subitem.timeStepDelai}
                  </DropdownToggle>
                  <DropdownMenu>
                    {demarcheSteps.timeSteps.map((timeStep, key) => (
                      <DropdownItem key={key} id={key} onClick={()=>this.setPropsValue(timeStep.texte, 'timeStepDelai')}>
                        {timeStep.texte}
                      </DropdownItem>
                    ))}
                  </DropdownMenu>
                ...
                <span className="color-grisFonce">Précisez le délai de réponse légal ou constaté.</span>
              </div>

              <div className={"row-config negative-padding" + (validatedRow[3] ? " validated" : "")}>
                <EVAIcon name="file-text" fill={variables.noir} className="mr-10 mb-10" />
                <b className="mr-10 mb-10">Documents justificatifs {subitem.papiers && subitem.papiers.length > 0 && ("(" + subitem.papiers.length + ") ")}:</b>
                
                {(subitem.papiers || []).map((doc, idx) => (
                  <ButtonDropdown isOpen={isPapiersDropdownOpen[idx]} toggle={()=>this.togglePapiersDropdown(idx)} className="etape-dropdown mr-10" key={idx}>
                    <DropdownToggle caret className="docs-dropdown">
                      <span className="idx-docs">{idx + 1}</span>
                      {doc.texte}
                      {!disableEdit && 
                        <EVAIcon onClick={e=> {e.stopPropagation(); this.addDoc(doc, idx, false)}} className="delete-icon cursor-pointer" name="close-circle" fill={variables.error} size="xlarge" />}
                    </DropdownToggle>
                    <DropdownMenu>
                      {demarcheSteps.papiers.map((papier, key) => (
                        <DropdownItem key={key} id={key} onClick={()=>this.addDoc(papier, idx)}>
                          {(key+1) + " - " + papier.texte}
                        </DropdownItem>
                      ))}
                    </DropdownMenu>
                ...
                <ButtonDropdown isOpen={isPapiersDropdownOpen[isPapiersDropdownOpen.length - 1]} toggle={()=>this.togglePapiersDropdown(isPapiersDropdownOpen.length - 1)} className="etape-dropdown mr-10">
                  <DropdownToggle caret className="docs-dropdown">
                  ...
                  <DropdownMenu>
                    {demarcheSteps.papiers.map((papier, key) => (
                      <DropdownItem key={key} id={key} onClick={()=>this.addDoc(papier)}>
                        {(key+1) + " - " + papier.texte}
                      </DropdownItem>
                    ))}
                  </DropdownMenu>
