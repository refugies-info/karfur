...
class UneVariante extends Component {
  ...
  componentDidMount() {
    this.setState({
      isMounted: true,
      allCase: this.props.admin ? [true, false] : [true],
    });
    if (this.props.variantes && this.props.variantes.length > 0) {
      const {
        ...
        topValue,
        ...bprops
      } = this.props.variantes[0];
      let newCriteres = [],
        validatedRow = [!!villes && villes.length > 0, !!ageTitle];
      customCriteres.forEach((x) => {
        if (x.query && bprops[x.query]) {
          newCriteres = [
            ...newCriteres,
            {
              ...x,
              options: x.options.map((y) => ({
                ...y,
                selected: bprops[x.query].includes(y.texte),
              })),
            },
          ];
      ...
      if (newCriteres.length === 0) {
        newCriteres = [customCriteres[0]];
        validatedRow = [...validatedRow, false];
      }
      this.setState({
        ...(villes && { villes }),
  ...
  onPlaceSelected = (place) => {
    ...
    const filteredNewVilles = [
      ...new Set(newVilles.map((x) => x.place_id)),
    ].map((x) => newVilles.find((y) => y.place_id === x));
    this.setState((pS) => ({
      villes: filteredNewVilles,
  ...
  removeVille = (idx) =>
    this.setState((pS) => ({ villes: pS.villes.filter((_, i) => i !== idx) }));
  toggleDropdown = (name, idx = null) =>
    this.setState((pS) => ({
      dropdowns: {
        ...pS.dropdowns,
        [name]:
          idx === null
            ? !pS.dropdowns[name]
            : pS.dropdowns[name].map((x, i) => (i === idx ? !x : x)),
      },
    }));
  changeCritere = (idx, target) =>
    this.setState((pS) => ({
      newCriteres: pS.newCriteres.map((x, i) =>
        i === target ? customCriteres[idx] : x
      ),
    }));
  selectCritere = (idx, target) =>
    this.setState(
      (pS) => ({
        newCriteres: pS.newCriteres.map((x, i) =>
          i === target
      ...
      () =>
        this.setState((pS) => ({
          validatedRow: pS.validatedRow.map((x, i) =>
            i === target + 2
              ? this.state.newCriteres[target].options
  ...
  changeTitle = (idx, value) =>
    this.setState((pS) => ({
      ageTitle: value,
      validatedRow: pS.validatedRow.map((x, i) => (i === 1 ? idx > 0 : x)),
    }));

  toggleCas = (idx) => {
    if (idx === this.state.allCase.length - 1) {
      if (!this.state.validatedRow.includes(true)) {
        return;
      }
  ...
  supprimer_cas = (idx) => {
    this.setState((pS) => ({
      allCase: pS.allCase.filter((_, i) => idx !== i),
    }));
  ...
  validateCriteres = (close = false, idx = 0) => {
    ...
    if (ageTitle === "Moins de ** ans") {
      bottomValue = -1;
    } else if (ageTitle === "Plus de ** ans") {
      topValue = 999;
    }
    let newVariante = {
    ...
    validatedRow.forEach((x, i) => {
      if (i > 1 && x) {
        newVariante[
          this.state.newCriteres[i - 2].query
        ] = this.state.newCriteres[i - 2].options
          .filter((y) => y.selected)
          .map((y) => y.texte);
      }
    });
              ...
              return (
                ...
                >
                  ...
                  <div className={"col-body " + "with-header"}>
                      ...
                      >
                        {villes.length === 0 && (
                          <span className="color-grisFonce mt-10">
                            Cette démarche s’applique aux habitants de :
                          </span>
                        )}
                        ...
                        {villes.map((ville, key) => (
                          <div
                            className="selected-ville ml-10"
                            key={ville.place_id}
                          >
                    ...
                    <div
                      className={
                        "critere mb-10" + (validatedRow[1] ? " validated" : "")
                      }
                    >
                      <h5 className="critere-title">
                        <EVAIcon
                      ...
                      <div className="critere-body justify-content-space-between">
                        ...
                        <ButtonDropdown
                          isOpen={this.state.dropdowns.age}
                          toggle={() => this.toggleDropdown("age")}
                        ...
                        >
                          ...
                          <DropdownMenu>
                            {["Tout âge confondu", ...filtres.audienceAge].map(
                              (option, key) => (
                                <DropdownItem
                                  key={key}
                                  onClick={() => this.changeTitle(key, option)}
                                >
                                  {option}
                                </DropdownItem>
                              )
                    ...
                    {newCriteres.map((customCritere, idx) => (
                      <div
                        className={
                          "critere mb-10" +
                          (validatedRow[idx + 2] ? " validated" : "")
                        }
                        key={idx}
                      >
                        <ButtonDropdown
                          isOpen={this.state.dropdowns.criteres[idx]}
                          toggle={() => this.toggleDropdown("criteres", idx)}
                          className="content-title"
                        >
                          <h5 className="mb-20">
                            <DropdownToggle caret>
                          ...
                          <DropdownMenu>
                            {customCriteres.map((option, key) => (
                              <DropdownItem
                                key={key}
                                onClick={() => this.changeCritere(key, idx)}
                              >
                                {option.texte}
                              </DropdownItem>
                            ))}
                          ...
                          <div className="critere-options">
                            {customCritere.options.map((option, key) => (
                              <FButton
                                ...
                                key={key}
                                className={
                                  "ml-10 mt-10 custom-critere-btn" +
                    ...
                    <div
                      className="critere add-critere cursor-pointer"
                      onClick={this.addCritere}
                    >
