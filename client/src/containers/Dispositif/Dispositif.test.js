...
describe("Dispositif", () => {
  ...
  it("renders without crashing", () => {
    ...
    expect(wrapper.find("h1").at(0).childAt(0).props().html).to.equal(
      "Titre informatif"
    );
  });
  ...
  it("calls componentDidMount", () => {
    ...
    shallow(
      ...
      .dive()
      .dive();
    expect(Dispositif.prototype.componentDidMount).to.have.property(
      "callCount",
      1
    );
      ...
      async (response) => {
        if (
          response.config.url.includes(
            "http://localhost:3000/dispositifs/get_dispositif"
          )
        ) {
