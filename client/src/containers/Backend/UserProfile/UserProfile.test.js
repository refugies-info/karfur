...
describe("UserProfile", () => {
  ...
  it("renders without crashing", () => {
    ...
    expect(wrapper.find("span.hideOnPhone").at(0).text()).to.equal(
      "Tables.Mon profil"
    );
  });
  ...
  it("calls componentDidMount", () => {
    ...
    shallow(
      ...
      .dive()
      .dive();
    expect(UserProfile.prototype.componentDidMount).to.have.property(
      "callCount",
      1
    );
  ...
  it("should receive users from real API", function (done) {
    axios.interceptors.response.use(
      (response) => {
        if (
          response.config.url.includes("http://localhost:3000/user/get_users")
        ) {
