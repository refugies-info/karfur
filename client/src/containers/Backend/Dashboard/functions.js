...
const execute_search = async function (queries, sort, numElements) {
  ...
  let date_min = new Date(
    Math.min.apply(
      null,
      data.map((arr) => new Date(_.get(arr, "0.created_at", null)))
    )
  );
  ...
  const date_max = new Date(
    Math.max.apply(
      null,
      data.map(
        (arr) => new Date(_.get(arr, arr.length - 1 + ".created_at", null))
      )
    )
  );
  let date_array = [],
  ...
  for (var i = 0; i <= numElements; i++) {
    var intermDate = new Date(
      date_min.getTime() +
        (i * (date_max.getTime() - date_min.getTime())) / numElements
    );
  ...
  const max_traffic = Math.max.apply(
    null,
    traffic_data.map((arr) => Math.max.apply(null, arr))
  );
  ...
  this.setState((pS) => ({
    mainChart: {
      ...
      datasets: pS.mainChart.datasets.map((x, i) => ({
        ...x,
        data: traffic_data[i],
        label: queries[i].app || queries[i].page,
      })),
    },
    max_traffic,
    mainChartOpts: {
...
const calculate_avg_time = async function () {
  ...
  unmountData = unmountData.filter((x) =>
    mountData.some((y) => x.value === y.value)
  );
  mountData = mountData.filter((x) =>
    unmountData.some((y) => x.value === y.value)
  );
  activeData = activeData.filter((x) =>
    idleData.some((y) => x.value === y.value)
  );
  idleData = idleData.filter((x) =>
    activeData.some((y) => x.value === y.value)
  );
  ...
  //On somme les dates
  const uSommeDates = unmountData
    .map((x) => x.created_at)
    .reduce((acc, curr) => (acc += moment(curr)), 0);
  const mSommeDates = mountData
    .map((x) => x.created_at)
    .reduce((acc, curr) => (acc += moment(curr)), 0);
  const aSommeDates = activeData
    .map((x) => x.created_at)
    .reduce((acc, curr) => (acc += moment(curr)), 0);
  const iSommeDates = idleData
    .map((x) => x.time)
    .reduce((acc, curr) => (acc += moment(curr * 1)), 0); //Je prends le temps où il est devenu inactif

  const averageTimeOnsite =
    (new Date(uSommeDates).getTime() -
      new Date(mSommeDates).getTime() -
      (new Date(aSommeDates).getTime() - new Date(iSommeDates).getTime())) /
    minMountingLength; // / (1000 * 60) //- (new Date(aSommeDates).getTime() - new Date(iSommeDates).getTime()) / minIdleLength
