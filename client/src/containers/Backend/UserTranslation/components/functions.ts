import { GetDispositifsWithTranslationAvancementResponse, GetNeedResponse, TraductionsStatus } from "api-types";
import { colors } from "colors";
import { ITypeContenu, AvailableLanguageI18nCode } from "types/interface";

const filterDataExpert = (data: GetDispositifsWithTranslationAvancementResponse[], isExpert: boolean) => {
  if (isExpert) return data;
  return data.filter((trad) => [TraductionsStatus.TO_TRANSLATE, TraductionsStatus.VALIDATED].includes(trad.tradStatus));
};
...
const getTradAmount = (data: GetDispositifsWithTranslationAvancementResponse[]) => {
  ...
  const nbATraduire = data.filter((trad) => trad.tradStatus === TraductionsStatus.TO_TRANSLATE).length;
  const nbAValider = data.filter((trad) => trad.tradStatus === TraductionsStatus.PENDING).length;
  const nbPubliees = data.filter((trad) => trad.tradStatus === TraductionsStatus.VALIDATED).length;
...
const getNbTradByTypeContenu = (data: GetDispositifsWithTranslationAvancementResponse[]) => {
  const nbDispositifs = data.filter((trad) => trad.typeContenu === "dispositif").length;
  const nbDemarches = data.filter((trad) => trad.typeContenu === "demarche").length;
