...
const injectAnimatedApi = fn => animatedApi = fn;
...
class Controller {
  ...
  update(args) {
    ...
    const _interpolateTo = interpolateTo(args),
    ...
    if (is.arr(to) || is.fun(to)) {
      ...
      let merge = {};
      Object.entries(to).forEach((_ref) => {
        let k = _ref[0],
            v = _ref[1];
  ...
  start(onEnd) {
    ...
    if (this.queue.length) {
      ...
      if (this.localQueue) {
        this.localQueue.forEach((_ref2) => {
          let _ref2$from = _ref2.from,
      ...
      queue.forEach((_ref3, index) => {
        let delay = _ref3.delay,
  ...
  diff(props) {
    ...
    this.animations = Object.entries(this.merged).reduce((acc, _ref6, i) => {
      let name = _ref6[0],
          value = _ref6[1];
...
const useSprings = (length, props) => {
  ...
  const ctrl = useRef();
  const isFn = is.fun(props); // The controller maintains the animation values, starts and tops animations
...
let mapKeys = (items, keys) => (typeof keys === 'function' ? items.map(keys) : toArray(keys)).map(String);
...
function diffItems(_ref8, props) {
  ...
  added.forEach((key, index) => {
    ...
    if (unique && deleted.find(d => d.originalKey === key)) deleted = deleted.filter(t => t.originalKey !== key); // TODO: trail shouldn't apply to the first item, no matter if it's enter, leave or update!
...
export { config, createAnimatedComponent as animated, interpolate$1 as interpolate, Globals, useSpring, useTrail, useTransition, useChain, useSprings };
